                    ...
                    <div class="col-lg-8 col-md-8 col-lg-offset-2 col-md-offset-2">
                        <form role="form" onsubmit="task(this)">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="alias" value="{{ $role->alias }}" required>
                            </div>
                            <div class="form-group">
                                        ...
                                        <div class="panel panel-color panel-info">
                                            <div class="panel-heading">
                                                <h3 class="panel-title">
                                                    <input id="{{ $permission->name }}" name="permissions[]" type="checkbox" value="{{ $permission->name }}" @if($role->hasPermissionTo($permission->name)) checked @endif>
                                            ...
                                            <div class="panel-body">
                                                @foreach($permission->sub_permission as $sub)
                                                    <div class="panel panel-color panel-success">
                                                        <div class="panel-heading">
                                                            <h3 class="panel-title">
                                                                <input id="{{ $sub->name }}" name="permissions[]" type="checkbox" value="{{ $sub->name }}" @if($role->hasPermissionTo($sub->name)) checked @endif>
                                                                <label for="{{ $sub->name }}">
                                                                    {{ $sub->display_name }}
                                                                </label>
                                                            </h3>
                                                                    ...
                                                                    @foreach($sub->sub_permission as $su)
                                                                        <div class="col-xs-4">
                                                                            <div class="checkbox checkbox-info m-b-15">
                                                                                <input id="{{ $su->name }}" name="permissions[]" type="checkbox" value="{{ $su->name }}" @if($role->hasPermissionTo($su->name)) checked @endif>
                                                                                <label for="{{ $su->name }}">
                                                                                    {{ $su->display_name }}
                                                                                </label>
                                                                            </div>
